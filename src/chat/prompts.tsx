    ...
    return (
      <>
        <AssistantMessage>
          ...
          'SYNC', 'BUILD', 'TEST', or 'UNKNOWN'. Do not provide explanation.
          <br />
          - SYNC: There's a merge conflict when applying Electron's patches
          <br />
          - BUILD: There's a compilation error from clang
          <br />
          - TEST: There's a test failure from Mocha
          <br />
          Return 'UNKNOWN' unless you're absolutely sure.
        </AssistantMessage>
        <UserMessage>
...
interface AnalyzeTestErrorProps extends BasePromptElementProps {
  chromiumRoot: vscode.Uri;
  errorText: string;
  previousChromiumVersion: string;
  newChromiumVersion: string;
...
export class AnalyzeTestErrorPrompt extends PromptElement<AnalyzeTestErrorProps> {
  override async prepare() {}
  ...
  async render(_state: void, _sizing: PromptSizing) {
    return (
      <>
        <AssistantMessage>
          Suggest a list of Chromium CLs that might be the cause of the user's
          test error after upgrading the Chromium version. Analyze each page of
          the Chromium log (using the provided tools) for the changes between
          {this.props.previousChromiumVersion} and
          {this.props.newChromiumVersion}. DO NOT GIVE UP until you have checked
          all pages of the log and no more commits are found. DO NOT ASK THE
          ...
          not skip page numbers.
          <br />
          For any commit in the log which might be the cause of the test error,
          request the full commit details (using the provided tools) to
          determine if it is causing the test error. ALWAYS mention which page
          of the log that commit is on. YOU MUST mention the page of the log the
          commit is on. If it looks like it causes the test error, stop checking
          the log and output the commit details. Do not stop for any other
          reason.
          <br />
          List Chromium CLs by the value of \`Reviewed-on\` in the commit
          message.

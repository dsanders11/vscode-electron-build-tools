...
describe.only("analyzeTestError", () => {
  describe("finds the upstream CL for a test error", () => {
      ...
      describe(fixture.cl, () => {
        for (const model of globalThis._testModels) {
          it(`using ${model.name}`, async function () {
            ...
            let continuation: AnalyzeTestErrorContinuation | undefined;

            do {
              const stream = new MockChatResponseStream();
              const tools = getPrivateTools(this.globalContext.extension);

              const result = await analyzeTestError(
